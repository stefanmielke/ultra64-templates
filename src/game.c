...
enum { DM_TITLE, DM_UPMENU, DM_TRIMASK, DM_POSITION, DM_END };
...
enum { TM_TITLE, TM_UPMENU, TM_TEXTURE, TM_SCALE, TM_END };
...
#define NUMMODES 4
		...
		if (menuon) {
			...
			if (item->type == MT_MULVALI || item->type == MT_MULVALF) {
				if (item->minp)
					item->min = *item->minp * item->inc;
				if (item->maxp)
					item->max = *item->maxp / item->inc;
				if (item == (mainmenu + MM_NEAR))
					if (item->max > NEAR_MAX)
						item->max = NEAR_MAX;
			...
			if (pad[0]->button & CONT_RIGHT) {
				if (item->type == MT_MENU || item->type == MT_UPMENU) {
					...
					item->val = MAX(item->min, MIN(item->max, item->val * item->inc));
				} else if (item->type == MT_ADDVALI || item->type == MT_ADDVALF) {
					item->val = MAX(item->min, MIN(item->max, item->val + item->inc));
		...
		if (mainmenu[MM_VIEWSCALE].val < 1.0) {
			...
			gDPFillRectangle(glistp++, 0, 0, SCREEN_WD - 1,
			...
			gDPFillRectangle(glistp++, 0, 0,
							 (SCREEN_WD / 2) - (SCREEN_WD / 2) * mainmenu[MM_VIEWSCALE].val,
							 SCREEN_HT - 1);
			gDPFillRectangle(
				glistp++,
				SCREEN_WD - (SCREEN_WD / 2) + (SCREEN_WD / 2) * mainmenu[MM_VIEWSCALE].val, 0,
				SCREEN_WD - 1, SCREEN_HT - 1);
		}
		 ...
		 * set up matrices
		 */
		guPerspectiveF(allmat, &perspnorm, 30.0, 320.0 / 240.0, mainmenu[MM_NEAR].val,
					   mainmenu[MM_FAR].val, 1.0);
		guPerspective(&(dynamicp->projection), &perspnorm, 30.0, 320.0 / 240.0,
					  mainmenu[MM_NEAR].val, mainmenu[MM_FAR].val, 1.0);

		if (mainmenu[MM_EYEMODE].val == 1.0 || mainmenu[MM_EYEMODE].val == 3.0) {
			guLookAtF(m2, pp.pos[0] + pp.eye[0], pp.pos[1] + pp.eye[1], pp.pos[2] + pp.eye[2],
					  pp.pos[0], pp.pos[1], pp.pos[2], 0.0, 1.0, 0.0);
			guLookAt(&(dynamicp->viewing), pp.pos[0] + pp.eye[0], pp.pos[1] + pp.eye[1],
					 pp.pos[2] + pp.eye[2], pp.pos[0], pp.pos[1], pp.pos[2], 0.0, 1.0, 0.0);
		} else {
			guLookAtF(m2, pp.pos[0] + pp.eye[0], pp.pos[1] + pp.eye[1], pp.pos[2] + pp.eye[2],
					  pp.pos[0], pp.pos[1] + ballp, pp.pos[2], 0.0, 1.0, 0.0);
			guLookAt(&(dynamicp->viewing), pp.pos[0] + pp.eye[0], pp.pos[1] + pp.eye[1],
					 pp.pos[2] + pp.eye[2], pp.pos[0], pp.pos[1] + ballp, pp.pos[2], 0.0, 1.0, 0.0);
		}

		guMtxCatF(m2, allmat, m1);

		guScale(&(dynamicp->modeling), pp.scalex, 1.0, pp.scalez);
		guScaleF(modmat, pp.scalex, 1.0, pp.scalez);
		...
		gSPTexture(glistp++, texmenu[TM_SCALE].val, texmenu[TM_SCALE].val, 0, G_TX_RENDERTILE,
				   G_ON);
		...
		gDPLoadTextureBlock(glistp++, loadtex, G_IM_FMT_RGBA, G_IM_SIZ_16b, 32, 32, 0, G_TX_WRAP,
		 ...
		 */
		{
			...
			minz = MIN(MIN(modmat[0][2] * (-50) + modmat[2][2] * (-50) + modmat[3][2],
					   ...
					   MIN(modmat[0][2] * (-50) + modmat[2][2] * (50) + modmat[3][2],
						   modmat[0][2] * (50) + modmat[2][2] * (-50) + modmat[3][2]));
			if (maxx >= (65536 / 2) || maxy >= (65536 / 2) || maxz >= (65536 / 2) ||
				minx <= (-65536 / 2) || miny <= (-65536 / 2) || minz <= (-65536 / 2))
				FONTCOLM(HILITE_COL)
			else
				FONTCOLM(FONT_COL)
			...
			if (maxx >= (65536 / 2) || maxy >= (65536 / 2) || maxz >= (65536 / 2) ||
				maxw >= (65536 / 2) || minx <= (-65536 / 2) || miny <= (-65536 / 2) ||
				minz <= (-65536 / 2) || minw <= (-65536 / 2)) {
				sprintf(str, "CC=32768 (%s%s%s%s%s%s%s%s)", (maxx >= (65536 / 2)) ? "+X" : "",
						(maxy >= (65536 / 2)) ? "+Y" : "", (maxz >= (65536 / 2)) ? "+Z" : "",
						(maxw >= (65536 / 2)) ? "+W" : "", (minx <= (-65536 / 2)) ? "-X" : "",
						(miny <= (-65536 / 2)) ? "-Y" : "", (minz <= (-65536 / 2)) ? "-Z" : "",
						(minw <= (-65536 / 2)) ? "-W" : "");
				FONTCOLM(HILITE_COL);
				SHOWFONT(&glistp, str, 25, 116);
			} else if (ABS(maxx - minx) >= (65536 / 2) || ABS(maxy - miny) >= (65536 / 2) ||
					   ABS(maxz - minz) >= (65536 / 2) || ABS(maxw - minw) >= (65536 / 2)) {
		...
		if (menuon) {
			...
			for (mp = menu; mp->type != MT_END; mp++) {
				if (mp == item)
					...
					FONTCOL(mp->color[0], mp->color[1], mp->color[2], mp->color[3])
